...
Let's evaluate the association between mortgage denial and ... 
...
```{r}
summary(HMDA$unemp)
HMDA_Q <- round(quantile(HMDA$unemp, probs = c(0.33, .66)), 2)

HMDA$unemp_cat <- as.factor(ifelse(HMDA$unemp < HMDA_Q[1], paste("<", HMDA_Q[1]),
                     ifelse(HMDA$unemp > HMDA_Q[2], paste(">", HMDA_Q[2]),
...
Last but not least we can set the reference category of our choice. In this case we choose “3.2 - 3.6” which is the most frequent category.

```{r}
table(HMDA$unemp_cat)
```


```{r}
HMDA$unemp_cat <- relevel(HMDA$unemp_cat, ref="3.2 - 3.6")
