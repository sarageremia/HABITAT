...
ggplot(  
  data = mortgage_data,  
  mapping = aes(x = pi_ratio, y = mortgage)) +  
  geom_point()
```
...
ggplot(  
  ...
  geom_point(alpha = .5) +
  geom_vline(xintercept = 0.4, color = "red", linetype = "dotted") +
  geom_smooth(    
    method = "lm",   
    se = FALSE)
```
...
We find that an increase in P/I ratio from 0.4 to 0.5 is predicted to increase the probability of denial by approximately 39.4%.
...
$$OR = \frac{odds(Y|X + 1)}{odds(Y|X)} = \frac{e^{\beta_0 + \beta_1 (X+1)}}{e^{\beta_0 + \beta_1 X}} = e^{\beta_1}$$
...
- For example, if the odds ratio represents the effect of the P/I ratio on mortgage approval, an odds ratio of 1.229 means that for every 0.01 increase in the P/I ratio, the odds of mortgage denial increase by approximately 22.9%.
...
We want to assess if there is racial discrimination in the mortgage market.
Let's evaluate the association between mortgage denial and afam 

```{r}
table(HMDA$deny, HMDA$afam)
...
In general, the model will always have 1 parameter for a binary variable. However, we have to choose which level the parameter should represent by deciding which group to code with 1. The choice should be made according to how we want the model to be interpreted. For example, for the treatment variable, 0 is usually the placebo or the control treatment and 1 is usually the experimental treatment. Alternatively, in the case of the explanatory variables, we could simply use a statistical criteria: for a better stability of the model it is always better to use the most frequent category as a reference.    
    
For example, it makes more sense to consider as the reference level of `deny` subjects who have had a mortgage denial. This will help with the interpretation of the results. Therefore, we create a new variable `deny01`:

```{r}
HMDA$deny01 <- ifelse(HMDA$deny == "yes", 1, 0)
HMDA$afam01 <- ifelse(HMDA$afam == "yes", 1, 0)
...
$$Odds(Y|X = 1) = \exp^{\beta_0 + \beta_1}, \,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,Odds(Y|X = 0) = \exp^{\beta_0}$$
    

$$OR = \frac{Odds(Y|X = 1)}{Odds(Y|X = 0)}=\frac{\exp^{\beta_0 + \beta_1}}{\exp^{\beta_0}}=\exp^{\beta_1}$$

$$ln(OR) = \beta_1$$

```{r}
mdl_logit <- glm(deny01 ~ afam01, 
                  data = HMDA, 
                  family = binomial(link = "logit"))
```

```{r}
summ_log <- summary(mdl_logit)

exp(summ_log$coefficients[2])
```



### Categorical X with K levels

Before starting to fit any regression model we have to properly code categorical variables.
This is usually done in the initial data analysis and the descriptive statistics phase.
...
We must transform the variable into a factor, assign labels to the factor levels and choose a reference level. The reference level will be the group all the others levels will be compared to. In fact when a factor variable with K levels is added to a model, R internally transforms it in K-1 dummy variables and a model parameter will be estimated for each of them. Again, is important we choose the reference level. In this case we have `lvrat` which we have to transform into a factor

```{r}
summary(HMDA$lvrat)
HMDA_Q <- round(quantile(HMDA$lvrat, probs = c(0.33, .66)), 2)

HMDA$lvrat_cat <- as.factor(ifelse(HMDA$lvrat < HMDA_Q[1], paste("<", HMDA_Q[1]), 
                     ifelse(HMDA$lvrat > HMDA_Q[2], paste(">", HMDA_Q[2]),
                                                           paste(HMDA_Q[1], "-", HMDA_Q[2]))))
...
$\text{Odds}(Y|X = < 3.2) = e^{\beta_0 + \beta_1 (D1 = 0) + \beta_2 (D2 = 0)}$

$\text{Odds}(Y|X = 3.2-3.6) = e^{\beta_0 + \beta_1 (D1 = 1) + \beta_2 (D2 = 0)}$

$\text{Odds}(Y|X = > 3.6) = e^{\beta_0 + \beta_1 (D1 =01) + \beta_2 (D2 = 1)}$


$\text{OR} = \frac{\text{Odds}(Y|X = 50-59)}{\text{Odds}(Y|X = < 50)} = \frac{e^{\beta_0 + \beta_1 (D1 = 1) + \beta_2 (D2 = 0)}}{e^{\beta_0 + \beta_1 (D1 = 0) + \beta_2 (D2 = 0)}} = \frac{e^{\beta_0 + \beta_1}}{e^{\beta_0}}$


$\text{OR} = \frac{\text{Odds}(Y|X = 60+)}{\text{Odds}(Y|X = < 50)} = \frac{e^{\beta_0 + \beta_1 (D1 = 0) + \beta_2 (D2 = 1)}}{e^{\beta_0 + \beta_1 (D1 = 0) + \beta_2 (D2 = 0)}} = \frac{e^{\beta_0 + \beta_2}}{e^{\beta_0}}$

```{r}
mdl_logit2 <- glm(deny01 ~ lvrat_cat, 
                  data = HMDA, 
                  family = binomial(link = "logit"))
summary(mdl_logit2)
```
...
Last but not least we can set the reference category of our choice. In this case we choose “> 0.8” which is the most frequent category.

```{r}
table(HMDA$lvrat_cat)
```


```{r}
HMDA$lvrat_cat <- relevel(HMDA$lvrat_cat, ref="> 0.8")
